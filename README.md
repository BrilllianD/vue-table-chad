...
- **[Home](https://brillliand.github.io/vue-table-chad/)**
- **[Docs](https://brillliand.github.io/vue-table-chad/getting-started)**
- **[Demo](https://brillliand.github.io/vue-table-chad/demo/)**
- **[npm](https://www.npmjs.com/package/@brillliand/vue-table-chad)**
...
```bash
npm install @brillliand/vue-table-chad   # or pnpm add / yarn add
```
...
Each page is one topic, and each has a matching view in `npm run demo` where the same thing runs. The
pages are also a VitePress site — `npm run docs:dev` serves it locally, `npm run docs:build` builds it.
...
(`npm run demo` in a checkout). It is generated from the doc comments in `src/`, so it cannot fall
behind the code.
...
Node **24** (`.nvmrc` is committed — run `nvm use`) and npm.

```bash
nvm use             # Node 24
npm install
npm run dev         # playground at http://localhost:5173
npm run demo        # full feature demo at http://localhost:5174
npm test            # vitest
npm run typecheck
npm run lint
npm run bench       # pipeline and interaction benchmarks
npm run build       # library -> dist/
npm run size        # bundle-size budget over dist/
npm run docs:dev    # the VitePress docs site, locally
npm run build:site  # docs + demo, as published -> docs/.vitepress/dist/
npm run build:docs  # the demo, folded into one self-contained page
make pack           # build + size check + npm pack -> the installable tarball
```

`npm run dev` is four short examples. `npm run demo` is the exhaustive one — 23 views, every
export, one view per feature area, each listing the API it uses. See
[`demo/README.md`](demo/README.md). Inside this repo, `@brillliand/vue-table-chad` is an alias onto
`src/index.ts`.
