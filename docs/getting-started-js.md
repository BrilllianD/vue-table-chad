...
# in your project
pnpm add file:../vue-table-chad/brillliand-vue-table-chad-0.1.0.tgz
```
...
- **`shallowRef`, not `ref`.** A plain `ref` deep-proxies the array *and* every row object in it, so
  ...
  1.6–1.9× on filter and sort — see the README's [quick start](../README.md#hold-rows-in-a-shallowref).
...
| Page | For |
| --- | --- |
| [Using it in another project](getting-started.md) | The TypeScript version of this page: full `DataTable` props and slots tables. |
| [The two contracts](contracts.md) | `QueryState` and `DataSource` — the two shapes everything is written against. |
| [Local and server data](data-sources.md) | Debouncing, races, facet scoping. |
| [Excel-style filters](filtering.md) | The checklist, condition rules, facets. |
| [Selection](selection.md) | Shift-ranges, the tri-state header, selecting more rows than are loaded. |
| [Grouping rows](grouping.md) | Bands, `groupMode`, aggregates, whole-table totals. |
